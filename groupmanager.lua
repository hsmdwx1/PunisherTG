...
if matches[1] == "صاحب گروه" and is_admin(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[1] == "حذف صاحب گروه" and is_admin(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[1] == "کمک مدیر" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[1] == "حذف کمک مدیر" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
 tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[1] == "فیلتر" and matches[2] and is_mod(msg)  then
	if not lang then
	text = '<b>Word :</b> <code>'..matches[2]..'</code> <b>Has been Added to Filtered Words ! !</b>'
else
text = 'کلمه ی : <code>'..matches[2]..'</code> به لیست فیلترینگ اضافه شد !'
end
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, text, 1, 'html')
redis:sadd('filters:'..msg.chat_id_, matches[2])
end
if matches[1] == "حذف فیلتر" and matches[2] and is_mod(msg)  then
	if not lang then
	text = '<b>Word :</b> <code>'..matches[2]..'</code> <b>Has been Removed From Filtered Words ! !</b>'
	else
	text = 'کلمه ی : <code>'..matches[2]..'</code> از لیست فیلتر ها پاک شد !'
	end
	tdcli.sendMessage(msg.chat_id_, msg.id_, 1, text, 1, 'html')
	redis:srem('filters:'..msg.chat_id_, matches[2])
end
...
if matches[1] == "تنظیمات2" then
return mutes(msg, target)
end
if matches[1] == "لیست کمک مدیران" then
return modlist(msg)
end
if matches[1] == "لیست صاحب گروه" and is_owner(msg) then
...
_بازکردن تمامی امکانات بالا_

بیصدا | همه | گیف | متن | عکس | ویدیو | صدا | ویس | استیکر | مخاطب | فوروارد | مکان | فایل | سرویس تلگرام | دکمه شیشه ای | بازی
_بیصدا کردن تمامی امکانات بالا_

باصدا | همه | گیف | متن | عکس | ویدیو | صدا | ویس | استیکر | مخاطب | فوروارد | مکان | فایل | سرویس تلگرام | دکمه شیشه ای | بازی
...
_ch:@blackandwhiteTM_]]
return text
end
--------------------- Welcome -----------------------
local lang = redis:get("gp_lang:"..msg.chat_id_)
----------------------------------------
if matches[1] == 'تنظیم ولکام' and matches[2] then
	if not lang then
